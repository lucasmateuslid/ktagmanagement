...
export const Tags = () => {
  ...
  const handleDelete = async (id: string, e: React.MouseEvent) => {
    // CRITICAL: Stop event from bubbling to card click
    e.preventDefault();
    e.stopPropagation();
    ...
    if (confirm(t('deleteConfirm'))) {
      try {
        ...
        // DO NOT call loadData() here to avoid race conditions with Firestore
      } catch (err) {
        console.error("Delete error:", err);
        addNotification('error', 'Error', 'Failed to delete tag.');
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    if (formData.id) {
      const confirmMessage = t('saveConfirm') || 'Are you sure you want to save changes to this tag?';
    ...
    setFormData({});
    addNotification('success', 'Saved', 'Tag saved successfully.');
  };
  ...
  const processCSV = async (text: string) => {
    ...
    loadData();
    addNotification('success', 'Import Complete', `${count} tags imported.`);
  };
    ...
    <div className="space-y-6">
      <div className="flex flex-col xl:flex-row justify-between items-start xl:items-center gap-4">
        <div>
           <div className="flex items-center gap-4">
            <h1 className="text-2xl font-bold text-zinc-900 dark:text-white">{t('tagManagement')}</h1>
            <span className="bg-zinc-200 dark:bg-zinc-800 text-zinc-600 dark:text-zinc-400 text-xs px-2 py-1 rounded-full">
              {tags.length}
            </span>
          </div>
        </div>
        ...
        <div className="flex flex-col md:flex-row gap-2 w-full xl:w-auto">
          ...
          <div className="relative flex-1 md:w-80">
            ...
            <input 
              ...
              className="w-full pl-10 pr-4 py-2 rounded-lg border border-zinc-300 dark:border-zinc-700 bg-white dark:bg-zinc-900 text-sm focus:ring-2 focus:ring-primary-500 outline-none text-zinc-900 dark:text-white"
            />
          </div>
          ...
          <div className="flex gap-2 w-full md:w-auto overflow-x-auto pb-1 md:pb-0">
            ...
            <button
              ...
              className="bg-emerald-600 hover:bg-emerald-700 text-white px-4 py-2 rounded-lg flex items-center gap-2 transition-colors whitespace-nowrap"
            >
      ...
      {tags.length > 0 && (
        <div className="flex items-center gap-2 mb-2 px-1">
          <button 
            ...
            className="flex items-center gap-2 text-sm text-zinc-500 hover:text-primary-600 transition-colors"
          >
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
        {filteredTags.map((tag) => {
          ...
          return (
            <div 
              key={tag.id} 
              className={`
                relative rounded-xl shadow-sm border p-6 flex flex-col justify-between transition-all duration-200 cursor-pointer group select-none
                ${isSelected 
                  ? 'bg-primary-50 border-primary-500 dark:bg-primary-900/20 dark:border-primary-500' 
                  : 'bg-white dark:bg-zinc-900 border-zinc-200 dark:border-zinc-800 hover:border-primary-300 dark:hover:border-primary-700'}
              `}
              onClick={(e) => {
                toggleSelect(tag.id);
              }}
            >
              <div className="absolute top-4 right-4 z-0">
                 <div className={`
                    ...
                    ${isSelected ? 'bg-primary-600 border-primary-600' : 'border-zinc-300 dark:border-zinc-600 bg-white dark:bg-zinc-800'}
                 `}>
              ...
              <div>
                ...
                <h3 className="text-lg font-bold text-zinc-900 dark:text-white mb-1 truncate pr-2">{tag.name}</h3>
                <p className="text-xs text-zinc-500 font-mono mb-4">SN: {tag.accessoryId}</p>
                
                <div className="space-y-2 text-sm text-zinc-600 dark:text-zinc-400 bg-zinc-50 dark:bg-zinc-800 p-3 rounded-md border border-zinc-100 dark:border-zinc-700">
                   {tag.macAddress && (
                    <div className="flex justify-between border-b border-zinc-200 dark:border-zinc-700 pb-1 mb-1">
                      <span className="font-semibold text-xs uppercase text-zinc-500">{t('macAddress')}</span>
                      <span className="font-mono text-xs">{tag.macAddress}</span>
                    </div>
                   )}
                   {linkedVehicle && (
                    <div className="flex justify-between border-b border-zinc-200 dark:border-zinc-700 pb-1 mb-1 text-emerald-600 dark:text-emerald-400">
                  ...
                  <div className="flex justify-col gap-1 flex-col">
                    <span className="font-semibold text-xs uppercase text-zinc-500">{t('hashedKey')}</span>
                    <span className="font-mono text-[10px] break-all text-zinc-500 line-clamp-1">{tag.hashedAdvKey}</span>
                  </div>
                </div>
              </div>

              <div className="mt-4 pt-4 border-t border-zinc-100 dark:border-zinc-800 flex justify-between items-center">
                 <span className="text-xs text-zinc-400">{new Date(tag.createdAt).toLocaleDateString()}</span>
                 
                 <div className="flex gap-1">
                    <button 
                      onClick={(e) => openEdit(tag, e)} 
                      className="p-2 hover:bg-zinc-100 dark:hover:bg-zinc-800 rounded-lg text-zinc-400 hover:text-primary-600 transition-colors z-30 cursor-pointer"
                      title="Edit"
                    >
                      <Edit2 size={16} />
                    </button>
                    <button 
                      ...
                      className="p-2 hover:bg-red-50 dark:hover:bg-red-900/20 rounded-lg text-zinc-400 hover:text-red-600 transition-colors z-30 cursor-pointer"
                      title="Delete"
                    >
                      <Trash2 size={16} />
                    </button>
        ...
        {filteredTags.length === 0 && (
          <div className="col-span-full py-12 text-center text-zinc-500 bg-white dark:bg-zinc-900 rounded-xl border border-dashed border-zinc-300 dark:border-zinc-700">
            {t('noTags')}
          </div>
        )}
      </div>
      ...
      {isModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm p-4">
          <div className="bg-white dark:bg-zinc-900 rounded-2xl w-full max-w-md p-6 shadow-2xl overflow-y-auto max-h-[90vh] border border-zinc-200 dark:border-zinc-800">
            ...
            <form onSubmit={handleSave} className="space-y-4">
              <div>
                <label className="block text-sm font-medium text-zinc-700 dark:text-zinc-300 mb-1">{t('tagName')}</label>
                <input
                  type="text"
                  required
                  value={formData.name || ''}
              ...
              <div>
                <label className="block text-sm font-medium text-zinc-700 dark:text-zinc-300 mb-1">{t('accessoryId')}</label>
                <input
                  type="text"
                  required
              ...
              <div>
                <label className="block text-sm font-medium text-zinc-700 dark:text-zinc-300 mb-1">{t('macAddress')}</label>
                <input
                  type="text"
              ...
              <div>
                <label className="block text-sm font-medium text-zinc-700 dark:text-zinc-300 mb-1">{t('hashedKey')}</label>
                <input
                  type="text"
                  required
              ...
              <div>
                <label className="block text-sm font-medium text-zinc-700 dark:text-zinc-300 mb-1">{t('privateKey')}</label>
                <input
                  type="password"
                  required
