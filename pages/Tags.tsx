  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    try {
        ...
        if (tag.type === 'XADTAG') {
            ...
            tag.id = registered.id;
            tag.accessoryId = registered.identifierNormalized;
            tag.imei = registered.imeiOriginal;
            tag.identifierNormalized = registered.identifierNormalized;
            tag.traccarDeviceId = registered.traccarDeviceId;
            tag.traccarDeviceName = registered.traccarDeviceName;
            tag.traccarPositionId = registered.traccarPositionId;
            tag.traccarStatus = registered.traccarStatus;
            tag.integrationStatus = registered.integrationStatus;
        } else {
  ...
  const processImport = async () => {
    ...
    try {
      if (importConfig.type === 'XADTAG') {
          ...
          const summary = await trackingApi.importCommit(rows);
