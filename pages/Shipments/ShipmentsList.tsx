...
export const ShipmentsList = () => {
  const { shipments, loading, deleteShipment, updateShipmentStatus } =
    useShipments();
  ...
  React.useEffect(() => {
    const syncTracking = async () => {
      ...
      try {
        ...
        const { token, env } = await melhorEnvioService.getValidToken();
            ...
            if (data.updates && data.updates.length > 0) {
              ...
              for (const update of data.updates) {
                const shipment = shipments.find(
                  (s) => s.melhorEnvio?.orderId === update.orderId,
                );
                if (shipment) {
                  ...
                  // If status changed to cancelado, free the items
                  if (
                    update.status === "cancelado" &&
                    shipment.status !== "cancelado"
                  ) {
                    for (const item of shipment.itens) {
                      if (item.tipo === "tag" && item.tagId) {
                        await updateItemStatus("tag", item.tagId, "disponível");
                      } else if (
                        item.tipo === "rastreador" &&
                        item.rastreadorId
                      ) {
              ...
              console.log(
                `[Sync] ${data.updates.length} remessas atualizadas.`,
              );
            }
          }
        }
  ...
  const handleTrackPackage = async (
    ...
    try {
      ...
      if (!apiKey) {
        throw new Error(
          "Chave da API do Site Rastreio não configurada. Configure-a nas configurações do sistema.",
        );
      }
      ...
      setTrackingError(
        err.message ||
          "Ocorreu um erro desconhecido ao tentar rastrear a encomenda.",
      );
    } finally {
  ...
  const handleCancel = (e: React.MouseEvent, id: string) => {
    ...
    setConfirmModal({
      ...
      message:
        "Tem certeza que deseja cancelar esta remessa? Esta ação pode ser irreversível. Itens vinculados retornarão ao estoque.",
      type: "warning",
      onConfirm: async () => {
        if (shipment) {
          for (const item of shipment.itens) {
        ...
        await updateShipmentStatus(id, "cancelado");
      },
    });
  };
  ...
  const handleDelete = (e: React.MouseEvent, id: string) => {
    ...
    setConfirmModal({
      ...
      message:
        "ATENÇÃO: Isso excluirá o registro permanentemente. Itens vinculados retornarão ao estoque. Continuar?",
      type: "danger",
      onConfirm: async () => {
        if (shipment) {
          for (const item of shipment.itens) {
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
          ...
          <p className="text-sm text-zinc-500">
            Controle de remessas e lotes de equipamentos
          </p>
        </div>
          ...
          >
            <div className="flex-1 space-y-2">
              <div className="flex items-center gap-3">
                ...
                <span
                  className={`px-3 py-1 rounded-full text-xs font-bold uppercase tracking-wider flex items-center gap-1 ${getStatusColor(shipment.status)}`}
                >
              ...
              <p className="text-xs text-zinc-400">
                Criado em:{" "}
                {new Date(shipment.dataCriacao).toLocaleDateString("pt-BR")} por{" "}
                {shipment.consultorNome}
              </p>
            </div>
            ...
            <div className="flex flex-col md:items-end gap-2 w-full md:w-auto">
              ...
              {shipment.codigoRastreio ? (
                <div className="flex items-center gap-2 bg-zinc-50 dark:bg-zinc-800/50 px-3 py-2 rounded-lg border border-zinc-200 dark:border-zinc-700">
                  ...
                  <button
                    onClick={(e) =>
                      handleTrackPackage(e, shipment.codigoRastreio!)
                    }
              ...
              {(canEdit ||
                (user?.id === shipment.consultorId &&
                  (shipment.status === "rascunho" ||
                    shipment.status === "em_analise"))) && (
                <div className="flex gap-2 mt-2">
                  <button
                    onClick={(e) => handleEdit(e, shipment.id)}
                    className="p-2 bg-zinc-100 dark:bg-zinc-800 rounded-lg text-zinc-600 dark:text-zinc-400 hover:bg-zinc-200 dark:hover:bg-zinc-700"
                    title="Editar"
                  >
                    <Edit2 size={16} />
                  </button>
                  {shipment.status !== "cancelado" && (
                    <button
                      onClick={(e) => handleCancel(e, shipment.id)}
                      className="p-2 bg-amber-100 dark:bg-amber-900/20 rounded-lg text-amber-600 dark:text-amber-400 hover:bg-amber-200 dark:hover:bg-amber-900/40"
                      title="Cancelar"
                    >
                      <XCircle size={16} />
                    </button>
                  ...
                  {canEdit && (
                    <button
                      ...
                      className="p-2 bg-red-100 dark:bg-red-900/20 rounded-lg text-red-600 dark:text-red-400 hover:bg-red-200 dark:hover:bg-red-900/40"
                      title="Excluir"
                    >
                      <Trash2 size={16} />
                    </button>
        ...
        {filteredShipments.length === 0 && (
          <div className="text-center py-12 bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-2xl">
            ...
            <p className="text-zinc-500">
              Tente ajustar os filtros ou crie uma nova remessa.
            </p>
          </div>
        )}
      </div>
