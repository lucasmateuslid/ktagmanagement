      ...
      storage.getSettings().then((s) => {
        if (s) {
          ...
          setFormData((prev) => ({
            ...prev,
            fromPostalCode:
              s.melhorEnvioSenderAddress?.postalCode || prev.fromPostalCode,
          }));
        }
      });
      setStep(1);
    ...
    try {
      ...
      if (!token) {
        addNotification(
          ...
          "O token da API do Melhor Envio não está configurado para o ambiente ativo.",
        );
        setLoading(false);
        return;
      }
      ...
      addNotification(
        "error",
        "Erro na cotação",
        error.message || "Falha ao conectar com Melhor Envio",
      );
      console.error(error);
    } finally {
      setLoading(false);
              ...
              {step === 1 ? (
                      ...
                      {settings?.melhorEnvioPackages &&
                        settings.melhorEnvioPackages.length > 0 && (
                          ...
                          >
                            ...
                            {settings.melhorEnvioPackages.map(
                              (pkg: ShippingPackage) => (
                    ...
                    <div className="grid grid-cols-2 gap-4">
                      <div className="space-y-2">
                        ...
                        <input
                          ...
                          className="w-full px-4 py-3 bg-white dark:bg-zinc-800 border border-zinc-200 dark:border-zinc-700 rounded-xl outline-none focus:border-emerald-500 font-medium"
                        />
                      </div>
                      <div className="space-y-2">
                        <label className="text-[10px] font-black uppercase tracking-widest text-zinc-500">
                ...
                <div className="space-y-4">
                  <div className="flex items-center gap-3 mb-6">
                    <button
                      onClick={() => setStep(1)}
                      className="text-zinc-500 hover:text-zinc-900 dark:hover:text-white text-sm font-medium"
                    >
                      Voltar
                    </button>
                  ...
                  {results.length > 0 ? (
                    <div className="space-y-3">
                      {results
                        ...
                        .sort(
                          (a, b) =>
                            Number(a.custom_price) - Number(b.custom_price),
                        )
                        .map((option, idx) => (
                          <div
                            key={idx}
                            className="flex items-center justify-between p-4 bg-zinc-50 dark:bg-zinc-800/50 rounded-2xl border border-zinc-200 dark:border-zinc-700/50"
                          >
                            <div className="flex items-center gap-4">
                              ...
                              <div>
                                <h4 className="font-bold text-zinc-900 dark:text-white leading-none">
                                  {option.company?.name || "Transportadora"} -{" "}
                                  {option.name}
                                </h4>
                                <p className="text-[11px] text-zinc-500 mt-1 font-medium">
                                  Entrega em até {option.custom_delivery_time}{" "}
                                  dias úteis
                                </p>
                              </div>
                            </div>
                            <div className="text-right">
                              ...
                              {option.price &&
                                option.price !== option.custom_price && (
                                  <div className="line-through text-[10px] text-zinc-400">
                                    R$ {option.price} no balcão
                                  </div>
                                )}
                            </div>
                          </div>
                ...
                >
                  {loading ? (
                    <div className="flex gap-1 items-center">
                      <Calculator className="animate-pulse" size={18} />
                      Calculando...
                    </div>
                  ) : (
                    <>
