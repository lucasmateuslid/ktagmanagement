...
async function request<T>(path: string, init: RequestInit = {}): Promise<T> { const token = await auth?.currentUser?.getIdToken(); if (!token) throw new Error('Autenticação necessária.'); const response = await fetch(path, { ...init, headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${token}`, 'X-Tenant-Id': activeTenant.id, ...(init.headers || {}) } }); const payload = await response.json().catch(() => ({})); if (!response.ok) throw new Error(payload.error || 'Falha no serviço de rastreamento.'); return payload.data as T; }
export const trackingApi = {
  ...
  websocket: async () => { const token = await auth?.currentUser?.getIdToken(); if (!token) throw new Error('Autenticação necessária.'); const scheme = location.protocol === 'https:' ? 'wss:' : 'ws:'; return new WebSocket(`${scheme}//${location.host}/ws/tracking?tenant=${encodeURIComponent(activeTenant.id)}`, [`firebase.${token}`]); },
};
