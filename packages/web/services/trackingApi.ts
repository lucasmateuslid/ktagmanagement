...
async function request<T>(path: string, init: RequestInit = {}): Promise<T> { const token = await auth?.currentUser?.getIdToken(); if (!token) throw new Error('Autenticação necessária.'); const url = trackingHttpUrl(path); let response: Response; try { response = await fetch(url, { ...init, headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${token}`, 'X-Tenant-Id': activeTenant.id, ...(init.headers || {}) } }); } catch (error) { if ((error as Error).name === 'AbortError') throw error; throw new TrackingApiError(`${friendlyHistoryError('NETWORK_ERROR')} (${new URL(url, window.location.origin).host})`, 'NETWORK_ERROR'); } const payload = await response.json().catch(() => ({})); if (!response.ok) { const code = String(payload.errorCode || 'TRACKING_UNAVAILABLE'); throw new TrackingApiError(friendlyHistoryError(code), code, payload.requestId, response.status); } return payload.data as T; }
