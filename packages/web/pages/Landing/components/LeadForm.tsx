...
import { formatPhone, isValidPhone, normalizePhone } from '@ktag/shared';
...
export function buildWhatsAppLeadUrl(lead: Lead) {
  const message = [
    ...
    `Nome: ${lead.name}`, `Empresa: ${lead.company}`, `WhatsApp: ${formatPhone(lead.phone)}`,
...
export default function LeadForm() {
  ...
  const submit = (event: FormEvent) => {
    ...
    if (!lead.name.trim() || !lead.company.trim() || !isValidPhone(phoneDigits) || !/^\S+@\S+\.\S+$/.test(lead.email) || !lead.vehicles) {
