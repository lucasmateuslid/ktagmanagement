// Carrega .env da raiz do monorepo (packages/backend/src/ → 3 níveis acima)
...
async function startServer() {
  ...
  app.post("/api/proxy", sensitiveLimiter, async (req, res) => {
    try {
      ...
      // K-TAG: o cliente envia apenas injectAuth:'ktag' (sem url nem Authorization);
      // o servidor resolve a URL e injeta o Basic Auth do env (paridade com a
      // Firebase proxyApi). As credenciais nunca chegam ao navegador.
      if (injectAuth === 'ktag') {
      ...
      // K-TAG: injeta o Basic Auth do env (o cliente nunca vê as credenciais).
      if (injectAuth === 'ktag') {
        const u = process.env.KTAG_API_USER || '';
        const p = process.env.KTAG_API_PASS || '';
        if (!u || !p) {
          return res.status(500).json({ error: 'Credenciais K-TAG ausentes no backend (KTAG_API_USER/PASS).' });
        }
