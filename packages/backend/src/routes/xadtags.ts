...
xadTagsRouter.post('/', async (req, res) => { try {
  ...
  const identifierOriginal = originalXadTagIdentifier(String(body.identifierOriginal ?? body.imei ?? ''));
  const traccarUniqueId = normalizeXadTagIdentifier(identifierOriginal);
  const result = await xadTagService.register({
    ...
    identifierKind: 'numeric_serial', identifierOriginal, identifierProfile: 'xadtag_legacy_numeric_10_to_15',
