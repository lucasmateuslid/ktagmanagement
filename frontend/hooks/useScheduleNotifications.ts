    ...
    const unsubscribe = storage.subscribeToSchedules('user', user.id, (schedules) => {
      schedules.forEach(schedule => {
        ...
        if (prevStatus && prevStatus !== schedule.status) {
          ...
          if (title) {
                        addNotification(type, title, msg, true, { audience: 'user', origin: 'schedule' });
            playSound(soundType);
          }
        }
        ...
        snapshot.docChanges().forEach((change) => {
            ...
            if (change.type === 'added') {
                if (!isFirstLoadRef.current) {
                    ...
                    if (isRecent) {
                        playSound('admin');
                        addNotification('info', 'Nova Solicitação', `Placa ${schedule.vehiclePlate} (${schedule.serviceType}) por ${schedule.requesterName}`, true, { audience: user.role, origin: 'schedule' });
                    }
                }
            }
            ...
            if (change.type === 'modified' && prevSchedule) {
                if (prevSchedule.status === 'Solicitada' && schedule.status === 'Em análise') {
                    ...
                    playSound('admin');
                    addNotification('info', 'Solicitação Assumida', `${whoAssumed} assumiu o agendamento de ${schedule.vehiclePlate}.`, true, { audience: user.role, origin: 'schedule' });
                }

                if (prevSchedule.status !== schedule.status) {
                    ...
                    switch (schedule.status) {
                        case 'Em orçamento':
                            ...
                            message = `Agendamento ${plate} entrou em orçamento.`;
                            break;
                        case 'Autorizada':
                            type = 'success';
                            title = 'Serviço Autorizado';
                            message = `Agendamento ${plate} foi autorizado.`;
                            break;
                        case 'Confirmada':
                            type = 'success';
                            title = 'Agenda Confirmada';
                            message = `Agendamento ${plate} foi confirmado para ${schedule.confirmedDate || 'data pendente'} ${schedule.confirmedTime || ''}.`;
                            break;
                        case 'Reagendada':
                            type = 'info';
                            title = 'Agenda Reagendada';
                            message = `Agendamento ${plate} foi reagendado.`;
                            break;
                        case 'Técnico no local':
                            type = 'success';
                            title = 'Técnico no Local';
                            message = `Técnico chegou para o atendimento de ${plate}.`;
                            break;
                        case 'Concluída':
                            type = 'success';
                            title = 'Serviço Concluído';
                            message = `Atendimento da placa ${plate} foi concluído.`;
                            break;
                        case 'Cancelada':
                            type = 'error';
                            title = 'Serviço Cancelado';
                            message = `Atendimento da placa ${plate} foi cancelado.`;
                            break;
                    }

                    if (type && title) {
                        addNotification(type, title, message, true, { audience: user.role, origin: 'schedule' });
                    }
                }
            }
    ...
    const timerInterval = setInterval(() => {
        ...
        pendingSchedulesRef.current.forEach(s => {
                ...
                if (ownerName && user.name === ownerName) {
                    ...
                    if (minutes >= 5 && (now - lastRemindTime) >= (5 * 60 * 1000)) {
                        ...
                        addNotification(
                            ...
                            true,
                            { audience: user.role, origin: 'schedule' }
                        );
            ...
            if (s.status === 'Solicitada' && minutes >= 5 && minutes < 6 && !notified5MinRef.current.has(s.id)) {
                playSound('admin');
                addNotification('info', 'Aguardando Atenção', `Solicitação '${s.vehiclePlate}' está pendente há 5 min.`, true, { audience: user.role, origin: 'schedule' });
        ...
        activeSchedulesRef.current.forEach(s => {
            ...
            if (dateStr && /^\d{4}-\d{2}-\d{2}$/.test(dateStr)) {
                ...
                if (hoursPassed >= 24 && !notified24hRef.current.has(s.id)) {
                    playSound('admin');
                    addNotification('error', 'Atenção Necessária', `O serviço ${s.vehiclePlate} passou de 24h do agendamento. Verifique se foi concluído ou cancelado.`, true, { audience: user.role, origin: 'schedule' });
