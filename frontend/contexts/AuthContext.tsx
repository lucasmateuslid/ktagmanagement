  ...
  const login = async (email: string, password?: string): Promise<string | void> => {
    ...
    try {
      if (backendApi.isConfigured()) {
        try {
          const response = await backendApi.login(email.toLowerCase().trim(), password || '');
          const backendUser = response.user as User;

          await storage.initEncryption(backendUser);
          const decryptedUser = await storage.findUserByEmail(backendUser.email, true);
          const finalUser = decryptedUser || backendUser;

          backendApi.setSessionToken(response.token);
          await storage.setSessionUser(finalUser);
          setUser(finalUser);
          rateLimitService.clear('login_attempt');
          return;
        } catch (backendError: any) {
          console.warn('Backend login unavailable, falling back to legacy auth:', backendError?.message || backendError);
        }
      }
