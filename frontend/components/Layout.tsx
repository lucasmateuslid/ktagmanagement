...
export const Layout = ({ children }: { children?: React.ReactNode }) => {
  ...
  useEffect(() => {
    if (user) {
        pushService.register(user.id, user.role);
    }
  }, [user]);
                                    ...
                                    <div key={n.id} className={`p-4 border-b border-zinc-100 dark:border-zinc-800/50 hover:bg-zinc-50 dark:hover:bg-zinc-950/50 transition-colors flex gap-3 ${!n.read ? 'bg-primary-50/50 dark:bg-primary-900/10' : ''}`}>
                                      ...
                                      <div className="flex-1">
                                          ...
                                          <div className="mt-2 flex items-center gap-2">
                                            <span className="text-[9px] text-zinc-300 block font-mono">{new Date(n.timestamp).toLocaleTimeString()}</span>
                                            <span className="text-[9px] text-zinc-400 uppercase tracking-wide">{n.deliveryStatus === 'lida' ? 'Lida' : 'Entregue'}</span>
                                          </div>
                                      </div>
                                  ...
                                  notifications.map(n => (
                                    <div key={n.id} className={`p-4 border-b border-zinc-800 flex gap-3 ${!n.read ? 'bg-zinc-800/50' : ''}`}>
                                      ...
                                      <div className="flex-1">
                                          ...
                                          <div className="mt-2 flex items-center gap-2">
                                            <span className="text-[9px] text-zinc-500 block font-mono">{new Date(n.timestamp).toLocaleTimeString()}</span>
                                            <span className="text-[9px] text-zinc-500 uppercase tracking-wide">{n.deliveryStatus === 'lida' ? 'Lida' : 'Entregue'}</span>
                                          </div>
                                      </div>
