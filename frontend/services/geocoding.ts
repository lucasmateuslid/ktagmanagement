...
export const geocodingService = {
  ...
  reverseGeocode: async (lat: number, lon: number): Promise<string> => {
    if (backendApi.isConfigured()) {
      try {
        const data = await backendApi.post<{ address?: string }>('/tracking/geocode/reverse', { lat, lon });
        if (data.address) {
          return data.address;
        }
      } catch (error) {
        console.warn('Backend geocoding unavailable, using fallback providers.', error);
      }
    }
