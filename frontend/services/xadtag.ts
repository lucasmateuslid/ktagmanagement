...
const callApi = async (
    ...
    if (backendApi.isConfigured()) {
        if (path === '/tag' && method === 'GET' && params?.ids) {
            return new Response(JSON.stringify(await backendApi.get('/tracking/xadtag/tag', {
                query: { id: params.ids }
            })), { status: 200 });
        }

        if (path === '/tag/history' && method === 'GET' && params?.Id && params?.TimeFrom && params?.TimeTo) {
            return new Response(JSON.stringify(await backendApi.get('/tracking/xadtag/history', {
                query: {
                    id: params.Id,
        ...
        if (path === '/tag' && method === 'PATCH' && Array.isArray(body)) {
            return new Response(JSON.stringify(await backendApi.patch('/tracking/xadtag/tag', { ids: body })), { status: 200 });
        }
    }
...
export const xadtagService = {
    ...
    activate: async (tag: Tag): Promise<boolean> => {
        ...
        try {
            ...
            if (backendApi.isConfigured()) {
                const data = await backendApi.patch('/tracking/xadtag/tag', { ids: [tag.traqcareId] });
