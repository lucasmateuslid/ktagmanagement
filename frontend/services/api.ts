...
export const fetchTagLocation = async (tag: Tag): Promise<KTagLocationResult[]> => {
  ...
  if (backendApi.isConfigured()) {
    try {
      const data = await backendApi.post<{ results?: any[] }>('/tracking/ktag/location', {
        accessoryId: tag.accessoryId,
        hashedKeys: [tag.hashedAdvKey].filter(Boolean),
        privateKeys: [tag.privateKey].filter(Boolean)
      });
      ...
      console.warn('Backend tracking unavailable, using legacy proxy fallback.', error);
    }
  }
