    ...
    if (backendApi.isConfigured()) {
      try {
        const [data, categories] = await Promise.all([
          backendApi.get<any>(`/integrations/plate/${cleanPlate}`),
          storage.getCategories()
        ]);

        const rawType = (data.typeHint || '').toString().toLowerCase();
        let matchedCatId = '';

        if (rawType.includes('moto') || rawType.includes('ciclo') || String(data.model || '').toLowerCase().includes('honda cg')) {
        ...
        console.warn('Backend plate lookup unavailable, using legacy fallback.', error);
      }
    }
