        ...
        if (backendApi.isConfigured()) {
            try {
                const [result, categories] = await Promise.all([
                    backendApi.post<any>('/integrations/hinova/search', { query: plateOrChassis }),
                    storage.getCategories()
                ]);

                if (!result) return null;

                const targetFipeType = result.vehicle?.typeHint || 'carros';
                ...
                console.warn('Backend Hinova unavailable, using legacy fallback.', error);
            }
        }
