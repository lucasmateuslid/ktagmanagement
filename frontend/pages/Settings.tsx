...
export const Settings = () => {
  ...
  const backendManagedSecrets = backendApi.isConfigured();
  ...
  const handleSaveSettings = async (e: React.FormEvent) => {
    ...
    if (!settings) return;
    try {
      const payload = backendManagedSecrets
        ? {
            ...settings,
            customProxyUrl: '',
            ktagUser: '',
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 md:gap-10">
        ...
        <div className="lg:col-span-8 space-y-6 md:space-y-10">
              ...
              <div className="bg-white dark:bg-zinc-900 p-6 md:p-10 rounded-[32px] md:rounded-[40px] border border-zinc-100 dark:border-zinc-800 shadow-sm space-y-8">
                ...
                {backendManagedSecrets && (
                  <div className="rounded-2xl border border-red-200 dark:border-red-900/40 bg-red-50 dark:bg-red-950/30 px-4 py-3 text-xs font-bold text-red-600 dark:text-red-300">
                    Chaves e credenciais operacionais devem ficar no backend. Use a variável pública do serviço para Maps e gerencie segredos via ambiente/Secret Manager.
                  </div>
                )}
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="relative">
                      <input 
                        ...
                        disabled={!isAdmin || backendManagedSecrets}
              ...
              <div className="bg-white dark:bg-zinc-900 p-6 md:p-10 rounded-[32px] md:rounded-[40px] border border-zinc-100 dark:border-zinc-800 shadow-sm space-y-8">
                ...
                {backendManagedSecrets && (
                  <div className="rounded-2xl border border-primary-200 dark:border-primary-900/40 bg-primary-50/60 dark:bg-primary-950/20 px-4 py-3 text-xs font-bold text-primary-700 dark:text-primary-300">
                    Backend conectado: URL, usuário e senha K-Tag agora devem ser configurados apenas no serviço backend.
                  </div>
                )}
                <div className="space-y-6">
                  <div className="space-y-2">
                    ...
                    <div className="relative">
                      ...
                      <input type="text" disabled={!isAdmin || backendManagedSecrets} value={isAdmin ? settings.ktagUrl : '••••••••••••••••'} onChange={e => setSettings({...settings, ktagUrl: e.target.value})} className="w-full pl-11 pr-5 py-4 bg-zinc-50 dark:bg-zinc-800 border border-zinc-200 dark:border-zinc-700 rounded-2xl font-bold text-xs outline-none focus:border-primary-500 disabled:opacity-50" placeholder="https://api.ktag.example.com" />
                    </div>
                  </div>
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      <div className="space-y-2">
                        <label className="text-[10px] font-black uppercase text-zinc-500 tracking-wider">Usuário K-Tag</label>
                        <input type="text" disabled={!isAdmin || backendManagedSecrets} value={backendManagedSecrets ? 'Gerenciado no backend' : isAdmin ? settings.ktagUser : '••••••••••••••••'} onChange={e => setSettings({...settings, ktagUser: e.target.value})} className="w-full px-5 py-4 bg-zinc-50 dark:bg-zinc-800 border border-zinc-200 dark:border-zinc-700 rounded-2xl font-bold text-xs outline-none focus:border-primary-500 disabled:opacity-50" />
                      </div>
                      <div className="space-y-2">
                        <label className="text-[10px] font-black uppercase text-zinc-500 tracking-wider">Senha K-Tag</label>
                        <div className="relative">
                          <input type={showKTagPass ? 'text' : 'password'} disabled={!isAdmin || backendManagedSecrets} value={backendManagedSecrets ? 'backend-managed-secret' : isAdmin ? settings.ktagPass : '••••••••••••••••'} onChange={e => setSettings({...settings, ktagPass: e.target.value})} className="w-full px-5 py-4 bg-zinc-50 dark:bg-zinc-800 border border-zinc-200 dark:border-zinc-700 rounded-2xl font-bold text-xs outline-none focus:border-primary-500 pr-12 disabled:opacity-50" />
                          {isAdmin && !backendManagedSecrets && <button type="button" onClick={() => setShowKTagPass(!showKTagPass)} className="absolute right-4 top-1/2 -translate-y-1/2 text-zinc-400">{showKTagPass ? <EyeOff size={16}/> : <Eye size={16}/>}</button>}
                        </div>
                      </div>
                  </div>
                </div>
              ...
              <div className="bg-white dark:bg-zinc-900 p-6 md:p-10 rounded-[32px] md:rounded-[40px] border border-zinc-100 dark:border-zinc-800 shadow-sm space-y-8">
                ...
                {backendManagedSecrets && (
                  <div className="rounded-2xl border border-emerald-200 dark:border-emerald-900/40 bg-emerald-50/60 dark:bg-emerald-950/20 px-4 py-3 text-xs font-bold text-emerald-700 dark:text-emerald-300">
                    Integração Hinova em modo seguro: token, usuário e senha ficam somente no backend.
                  </div>
                )}
                <div className="space-y-6">
                  <div className="space-y-2">
                    <label className="text-[10px] font-black uppercase text-zinc-500 tracking-wider">URL do Endpoint SGA</label>
                    <input type="text" disabled={!isAdmin || backendManagedSecrets} value={isAdmin ? settings.hinovaUrl : '••••••••••••••••'} onChange={e => setSettings({...settings, hinovaUrl: e.target.value})} className="w-full px-5 py-4 bg-zinc-50 dark:bg-zinc-800 border border-zinc-200 dark:border-zinc-700 rounded-2xl font-bold text-xs disabled:opacity-50" placeholder="https://api.hinova.com.br/api/sga/v2" />
                  </div>
                  <div className="space-y-2">
                    <label className="text-[10px] font-black uppercase text-zinc-500 tracking-wider">Token SGA (Master / SGA Token)</label>
                    <div className="relative">
                      <input type={showHinovaToken ? 'text' : 'password'} disabled={!isAdmin || backendManagedSecrets} value={backendManagedSecrets ? 'backend-managed-secret' : isAdmin ? settings.hinovaToken : '••••••••••••••••'} onChange={e => setSettings({...settings, hinovaToken: e.target.value})} className="w-full px-5 py-4 bg-zinc-50 dark:bg-zinc-800 border border-zinc-200 dark:border-zinc-700 rounded-2xl font-mono text-[10px] outline-none focus:border-emerald-500 pr-12 disabled:opacity-50" />
                      {isAdmin && !backendManagedSecrets && <button type="button" onClick={() => setShowHinovaToken(!showHinovaToken)} className="absolute right-4 top-1/2 -translate-y-1/2 text-zinc-400">{showHinovaToken ? <EyeOff size={16}/> : <Eye size={16}/>}</button>}
                    </div>
                  </div>
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      <div className="space-y-2">
                        <label className="text-[10px] font-black uppercase text-zinc-500 tracking-wider">Usuário de Autenticação</label>
                        <input type="text" disabled={!isAdmin || backendManagedSecrets} value={backendManagedSecrets ? 'Gerenciado no backend' : isAdmin ? settings.hinovaUser : '••••••••••••••••'} onChange={e => setSettings({...settings, hinovaUser: e.target.value})} className="w-full px-5 py-4 bg-zinc-50 dark:bg-zinc-800 border border-zinc-200 dark:border-zinc-700 rounded-2xl font-bold text-xs disabled:opacity-50" />
                      </div>
                      <div className="space-y-2">
                        <label className="text-[10px] font-black uppercase text-zinc-500 tracking-wider">Senha de Autenticação</label>
                        <div className="relative">
                          <input type={showHinovaPass ? 'text' : 'password'} disabled={!isAdmin || backendManagedSecrets} value={backendManagedSecrets ? 'backend-managed-secret' : isAdmin ? settings.hinovaPass : '••••••••••••••••'} onChange={e => setSettings({...settings, hinovaPass: e.target.value})} className="w-full px-5 py-4 bg-zinc-50 dark:bg-zinc-800 border border-zinc-200 dark:border-zinc-700 rounded-2xl font-bold text-xs disabled:opacity-50" />
                          {isAdmin && !backendManagedSecrets && <button type="button" onClick={() => setShowHinovaPass(!showHinovaPass)} className="absolute right-4 top-1/2 -translate-y-1/2 text-zinc-400">{showHinovaPass ? <EyeOff size={16}/> : <Eye size={16}/>}</button>}
                        </div>
                      </div>
                  </div>
                </div>
              ...
              <div className="bg-white dark:bg-zinc-900 p-6 md:p-10 rounded-[32px] md:rounded-[40px] border border-zinc-100 dark:border-zinc-800 shadow-sm space-y-8">
                ...
                {backendManagedSecrets && (
                  <div className="rounded-2xl border border-cyan-200 dark:border-cyan-900/40 bg-cyan-50/60 dark:bg-cyan-950/20 px-4 py-3 text-xs font-bold text-cyan-700 dark:text-cyan-300">
                    O proxy legado do Firebase será descontinuado. A nova borda oficial é a API backend definida por ambiente.
                  </div>
                )}
                <div className="space-y-4">
                  <div className="space-y-2">
                    ...
                    <div className="relative">
                      ...
                      <input type="text" disabled={!isAdmin || backendManagedSecrets} value={backendManagedSecrets ? 'Gerenciado pela variável VITE_BACKEND_API_URL' : isAdmin ? settings.customProxyUrl : '••••••••••••••••'} onChange={e => setSettings({...settings, customProxyUrl: e.target.value})} className="w-full pl-12 pr-5 py-4 bg-zinc-50 dark:bg-zinc-800 border border-zinc-200 dark:border-zinc-700 rounded-2xl font-mono text-[10px] outline-none disabled:opacity-50" />
                    </div>
              ...
              <div className="bg-white dark:bg-zinc-900 p-6 md:p-10 rounded-[32px] md:rounded-[40px] border border-zinc-100 dark:border-zinc-800 shadow-sm space-y-8">
                ...
                {backendManagedSecrets && (
                  <div className="rounded-2xl border border-primary-200 dark:border-primary-900/40 bg-primary-50/60 dark:bg-primary-950/20 px-4 py-3 text-xs font-bold text-primary-700 dark:text-primary-300">
                    Token Traqcare em transição para backend. O campo local fica bloqueado para evitar novo vazamento em Firestore/UI.
                  </div>
                )}
                <div className="space-y-4">
                  <div className="space-y-2">
                    ...
                    <div className="relative">
                      <input 
                        ...
                        disabled={!isAdmin || backendManagedSecrets}
                        value={backendManagedSecrets ? 'backend-managed-secret' : isAdmin ? settings.traqcareToken : '••••••••••••••••'} 
                      ...
                      />
                      {isAdmin && !backendManagedSecrets && <button type="button" onClick={() => setShowTraqToken(!showTraqToken)} className="absolute right-4 top-1/2 -translate-y-1/2 text-zinc-400">{showTraqToken ? <EyeOff size={16}/> : <Eye size={16}/>}</button>}
                    </div>
