...
import { fetchTagLocation, ktagBatteryStatus } from '../services/api';
...
export const Tags = () => {
  ...
  const handleTestConnection = async (tag: Tag | null = activeTestTag) => {
      ...
      try {
          const locations = await fetchTagLocation(tag);
          addLog({
              type: 'success',
              status: 200,
              responseBody: {
                  pointsFound: locations.length,
                  latestPoint: locations[0] || null
              }
          });
      } catch (e: any) {
