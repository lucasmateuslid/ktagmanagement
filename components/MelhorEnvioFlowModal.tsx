...
export const MelhorEnvioFlowModal = ({
  ...
  const [step, setStep] = useState<
    "QUOTE" | "SELECT" | "CART" | "CHECKOUT" | "GENERATE" | "PRINT" | "DONE"
  >("QUOTE");
  ...
  const [melhorEnvioOrderId, setMelhorEnvioOrderId] = useState<string | null>(
    null,
  );
  ...
  useEffect(() => {
    if (isOpen && shipment) {
      ...
      const cepMatch =
        shipment.destinatario?.enderecoCompleto?.match(/\d{5}-\d{3}/);
      if (cepMatch) {
  ...
  const getToken = async () => {
    const { token } = await melhorEnvioService.getValidToken();
    return token;
  };
  ...
  const handleQuote = async () => {
    if (!fromPostalCode || !toPostalCode) {
      addNotification(
        "info",
        "Atenção",
        "Preencha os CEPs de origem e destino",
      );
      return;
    }
    ...
    try {
      ...
      const payload = {
        ...
        products: [
          {
            id: "x",
            width: Number(packageParams.width),
            height: Number(packageParams.height),
            length: Number(packageParams.length),
            weight: Number(packageParams.weight),
            insurance_value: Number(packageParams.insuranceValue),
            quantity: Number(packageParams.quantity),
          },
        ],
      };
  ...
  const handleAddToCart = async () => {
    ...
    try {
      ...
      const payload = {
        ...
        agency: selectedOption.company?.name?.toLowerCase().includes("jadlog")
          ? 1
          : null, // Simplificado, ideal é buscar agencia
        from: {
          ...fromAddress,
        ...
        to: {
          ...
          address:
            shipment?.destinatario?.enderecoCompleto?.split(",")[0] || "Rua",
          number: "S/N",
          district: "Centro",
          city: "São Paulo",
          state_abbr: "SP",
        ...
        products: [
          {
            name: "Equipamentos (Rastreador/Tag)",
            quantity: 1,
            unitary_value: Number(packageParams.insuranceValue),
          },
        ],
        volumes: [
          {
      ...
      addNotification(
        "success",
        "Sucesso",
        "Adicionado ao carrinho do Melhor Envio",
      );
    } catch (err: any) {
      addNotification("error", "Erro", err.message);
    } finally {
  ...
  const handleCheckout = async () => {
    ...
    try {
      ...
      if (!res.ok)
        throw new Error(
          data.error ||
            "Erro no checkout. Verifique saldo na sua carteira Melhor Envio.",
        );
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      addNotification(
        "success",
        "Gerada",
        "Etiqueta gerada com rastreio: " + tracking,
      );
      setStep("PRINT");
    } catch (err: any) {
      addNotification("error", "Erro", err.message);
    ...
    <AnimatePresence>
      <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
        ...
        <motion.div
          ...
          className="relative w-full max-w-2xl bg-white dark:bg-zinc-900 rounded-[32px] overflow-hidden shadow-2xl z-10 flex flex-col max-h-[90vh]"
        >
              ...
              <div className="space-y-6">
                <div className="bg-emerald-50 dark:bg-emerald-500/5 p-4 rounded-2xl border border-emerald-100 dark:border-emerald-500/10">
                  <p className="text-xs text-emerald-700 dark:text-emerald-400 font-medium">
                    Informe os CEPs e medidas do pacote para cotar os valores
                    com o Melhor Envio.
                  </p>
                </div>
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="relative">
                      ...
                      <input
                        ...
                        onChange={(e) =>
                          setFromPostalCode(e.target.value.replace(/\D/g, ""))
                        }
                        maxLength={8}
                        placeholder="00000000"
                        className="w-full pl-10 pr-4 py-3 bg-zinc-50 dark:bg-zinc-800 border border-zinc-200 dark:border-zinc-700 rounded-xl outline-none focus:border-emerald-500 font-medium"
                      />
                    </div>
                  </div>
                  <div className="space-y-2">
                    <label className="text-[10px] font-black uppercase tracking-widest text-zinc-500">
                      CEP Destino (Destinatário)
                    </label>
                    <div className="relative">
                      <MapPin
                        className="absolute left-4 top-1/2 -translate-y-1/2 text-zinc-400"
                      ...
                      <input
                        ...
                        value={toPostalCode}
                        onChange={(e) =>
                          setToPostalCode(e.target.value.replace(/\D/g, ""))
                        }
                        maxLength={8}
                        placeholder="00000000"
                        className="w-full pl-10 pr-4 py-3 bg-zinc-50 dark:bg-zinc-800 border border-zinc-200 dark:border-zinc-700 rounded-xl outline-none focus:border-emerald-500 font-medium"
                ...
                <div className="bg-zinc-50 dark:bg-zinc-800/50 p-4 rounded-2xl border border-zinc-200 dark:border-zinc-700/50">
                  <div className="flex items-center justify-between mb-4">
                    ...
                    {settings?.melhorEnvioPackages &&
                      settings.melhorEnvioPackages.length > 0 && (
                        <select
                          ...
                          onChange={(e) => {
                            const pkg = settings.melhorEnvioPackages.find(
                  ...
                  <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
                    <div className="space-y-2">
                      ...
                      <input
                        ...
                        className="w-full px-4 py-3 bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-700 rounded-xl outline-none focus:border-emerald-500 font-medium"
                      />
                    </div>
                    <div className="space-y-2">
                      <label className="text-[10px] font-black uppercase tracking-widest text-zinc-500">
                    ...
                    <div className="space-y-2">
                      ...
                      <input
                        type="number"
                        value={packageParams.insuranceValue}
                        onChange={(e) =>
                          setPackageParams({
                            ...packageParams,
            ...
            {step === "SELECT" && (
              <div className="space-y-4">
                <button
                  ...
                  className="text-zinc-500 text-sm hover:underline flex items-center gap-1 mb-4"
                >
                  <X size={14} /> Refazer Cotação
                </button>
                ...
                <div className="space-y-3">
                  {results
                    .filter((r) => !r.error)
                    .sort(
                      (a, b) => Number(a.custom_price) - Number(b.custom_price),
                    )
                    .map((option, i) => (
                      <div
                        key={i}
                        onClick={() => setSelectedOption(option)}
                        className={`p-4 rounded-2xl border cursor-pointer flex items-center justify-between transition-colors ${selectedOption?.id === option.id ? "border-emerald-500 bg-emerald-50 dark:bg-emerald-500/10" : "border-zinc-200 dark:border-zinc-700 bg-zinc-50 dark:bg-zinc-800 hover:border-zinc-300"}`}
                      >
                        <div className="flex gap-4 items-center">
                          ...
                          <div>
                            <h4 className="font-bold text-zinc-900 dark:text-white leading-tight">
                              {option.company?.name || "Transp"} - {option.name}
                            </h4>
                            <p className="text-xs text-zinc-500 mt-1">
                              Entrega em até {option.custom_delivery_time} dias
                              úteis
                            </p>
                          </div>
                        </div>
                  ...
                  {results.filter((r) => !r.error).length === 0 && (
                    <p className="text-zinc-500 text-center py-6">
                      Nenhuma opção disponível para este trecho.
                    </p>
                  )}
                </div>
                ...
                {selectedOption && (
                  <button
                    ...
                    disabled={loading}
                    className="w-full mt-4 bg-emerald-500 hover:bg-emerald-400 text-white dark:text-black py-4 rounded-xl flex items-center justify-center gap-2 font-black uppercase tracking-widest text-xs transition-colors"
                  >
                    {loading ? (
                      <Loader2 className="animate-spin" size={18} />
                    ) : (
              ...
              <div className="py-8 space-y-6 text-center flex flex-col items-center">
                ...
                <div className="space-y-2">
                  ...
                  <p className="text-zinc-500 text-sm max-w-sm mx-auto">
                    Para gerar o rastreio, o valor de{" "}
                    <b>
                      R${" "}
                      {selectedOption?.custom_price ||
                        shipment?.melhorEnvio?.price}
                    </b>{" "}
                    será debitado da sua carteira do Melhor Envio.
                  </p>
                </div>
                ...
                <button
                  ...
                  disabled={loading}
                  className="w-full max-w-sm bg-emerald-500 hover:bg-emerald-400 text-white dark:text-black py-4 rounded-xl flex items-center justify-center gap-2 font-black uppercase tracking-widest text-xs transition-colors"
                >
                  {loading ? (
                    <Loader2 className="animate-spin" size={18} />
                  ) : (
            ...
            {step === "GENERATE" && (
              <div className="py-8 space-y-6 text-center flex flex-col items-center">
                ...
                <div className="space-y-2">
                  ...
                  <p className="text-zinc-500 text-sm max-w-sm mx-auto">
                    Sua etiqueta está paga. Agora podemos gerar o PDF de
                    impressão e rastreio Oficial.
                  </p>
                </div>
                ...
                <button
                  ...
                  disabled={loading}
                  className="w-full max-w-sm bg-zinc-900 hover:bg-zinc-800 text-white py-4 rounded-xl flex items-center justify-center gap-2 font-black uppercase tracking-widest text-xs transition-colors"
                >
                  {loading ? (
                    <Loader2 className="animate-spin" size={18} />
                  ) : (
            ...
            {step === "PRINT" && (
              <div className="py-8 space-y-6 text-center flex flex-col items-center">
                ...
                <div className="space-y-2">
                  ...
                  <p className="text-zinc-500 text-sm max-w-sm mx-auto">
                    Código de Rastreio: <b>{trackingCode}</b>
                  </p>
                </div>
                ...
                <div className="flex gap-4 w-full max-w-sm">
                  {printUrl && (
                    <a
                      ...
                      className="flex-1 bg-primary-500 hover:bg-primary-400 text-black py-4 rounded-xl flex items-center justify-center gap-2 font-black uppercase tracking-widest text-[10px] transition-colors"
                    >
                  ...
                  <button
                    ...
                    className="flex-1 bg-zinc-100 dark:bg-zinc-800 text-zinc-900 dark:text-white py-4 rounded-xl flex items-center justify-center gap-2 font-black uppercase tracking-widest text-[10px] transition-colors"
                  >
                    Concluir
                  </button>
                </div>
