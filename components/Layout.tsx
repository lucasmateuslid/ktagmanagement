    ...
    const rawSections = [
      ...
      },
      {
        ...
        items: [
          ...
          { label: 'CRM / KANBAN', path: '/crm', icon: LayoutGrid, perm: 'ROUTE_SCHEDULES' },
    ...
    <div className="flex h-screen print:h-auto bg-zinc-100 dark:bg-black text-zinc-900 dark:text-zinc-100 overflow-hidden print:overflow-visible font-sans transition-colors duration-300 flex-col relative">
      ...
      <AnimatePresence>
          {activeToast && (
            ...
            >
              <div className={`
                p-4 rounded-xl shadow-2xl border backdrop-blur-md flex items-start gap-4 text-white
                ${activeToast.type === 'error' ? 'bg-zinc-900 border-zinc-800 border-l-4 border-l-red-500' : 
                  activeToast.type === 'success' ? 'bg-zinc-900 border-zinc-800 border-l-4 border-l-emerald-500' : 
                  'bg-zinc-900 border-zinc-800 border-l-4 border-l-blue-500'}
              `}>
                <div className="mt-0.5">
                  {activeToast.type === 'success' && <CheckCircle2 size={18} className="text-emerald-500" />}
                  {activeToast.type === 'error' && <XCircle size={18} className="text-red-500" />}
                  {activeToast.type === 'info' && <Bell size={18} className="text-blue-500" />}
                </div>
                <div className="flex-1">
                    <h4 className="font-bold text-sm tracking-tight mb-0.5">{activeToast.title}</h4>
                    <p className="text-xs font-medium text-zinc-400 leading-snug">{activeToast.message}</p>
                </div>
                <button onClick={closeToast} className="text-zinc-500 hover:text-white transition-colors"><X size={16} /></button>
              </div>
            </MotionDiv>
          )}
      </AnimatePresence>
