...
import { TechnicianTaskModal } from "./TechnicianTaskModal";
...
export const TrackingModal: React.FC<TrackingModalProps> = ({
  ...
  const [scannerActive, setScannerActive] = useState<"imei" | "tag" | null>(
    null,
  );
  ...
  const [formData, setFormData] = useState({
    ...
    checklist:
      schedule.checklist && schedule.checklist.length > 0
        ? schedule.checklist
        : defaultChecklistItems.map((name) => ({
            name,
            before: "" as ChecklistStatus,
            after: "" as ChecklistStatus,
          })),
  });

  useEffect(() => {
    if (
      formData.deviceType === "Tag" ||
      formData.deviceType === "Rastreador + Tag"
    ) {
      storage.getTags().then((tags) => {
        setAvailableTags(tags.filter((t) => t.status === "disponível"));
      });
    }
  }, [formData.deviceType]);
  ...
  const tagOptions = [
    ...availableTags.map((t) => ({
      id: t.traqcareId || t.imei || t.id,
      label: t.name,
      subLabel: t.traqcareId || t.imei,
    })),
    { id: "MANUAL", label: "+ Digitar ID Manualmente" },
  ];

  const completeService = async (createRegistration: boolean) => {
    try {
      if (createRegistration) {
        ...
        let clientExists = schedule.clientCpf
          ? clients.some((c) => c.cpf === schedule.clientCpf)
          : schedule.clientName
            ? clients.some((c) => c.name === schedule.clientName)
            : true;
        let vehicleExists = vehicles.some(
          (v) => v.plate === schedule.vehiclePlate,
        );

        let clientId = undefined;
        if (!clientExists && schedule.clientName) {
          clientId = crypto.randomUUID();
          await storage.saveClient({
            id: clientId,
            name: schedule.clientName,
            cpf: schedule.clientCpf || "",
            phone: schedule.clientPhone || "",
            email: schedule.clientEmail || "",
            address: schedule.clientAddress || "",
            hasAccess: false,
            createdAt: Date.now(),
          });
        }

        if (!vehicleExists && schedule.vehiclePlate) {
          await storage.saveVehicle({
            id: crypto.randomUUID(),
            plate: schedule.vehiclePlate,
            model: schedule.vehicleModel,
            clientId: clientId,
            type: "carros",
            status: "active",
            createdAt: Date.now(),
      ...
      const updated = {
        ...
        history: [
          ...schedule.history,
          {
            ...
            timestamp: Date.now(),
            details:
              "Vínculo de equipamento confirmado pelo consultor, finalizando serviço",
          },
        ],
      };
      onUpdate(updated);
      ...
      console.error(e);
      addNotification("error", "Erro", "Falha ao concluir serviço");
    } finally {
      setConfirmRegistrationOpen(false);
    }
  };

  const handleConfirmarEquipamentoClick = async () => {
    try {
      const [clients, vehicles] = await Promise.all([
        storage.getClients(),
        storage.getVehicles(),
      ...
      console.error(e);
      addNotification("error", "Erro", "Falha ao verificar cadastros");
    }
  };
  ...
  useEffect(() => {
    if (scannerActive) {
      scannerRef.current = new Html5Qrcode(
        scannerActive === "imei" ? "readerImei" : "readerTag",
      );
      scannerRef.current.start(
        ...
        (decodedText) => {
          if (scannerActive === "imei")
            setFormData({ ...formData, installedImei: decodedText });
          else setFormData({ ...formData, installedTagImei: decodedText });

          if (scannerRef.current && scannerRef.current.isScanning) {
            scannerRef.current
              .stop()
              .then(() => setScannerActive(null))
              .catch(console.error);
          } else {
  ...
  const handleHinovaLookup = async () => {
    ...
    if (
      ...
      !/^[A-Z]{3}[0-9][A-Z0-9][0-9]{2}$/.test(formData.vehiclePlate)
    ) {
      addNotification(
        "info",
        "Validação",
  ...
  const generateOsPdf = (type: "os" | "receipt") => {
    ...
    autoTable(doc, {
      ...
      body: [
        ...
        [
          "Veículo",
          `${schedule.vehicleModel || "N/A"} - ${schedule.vehiclePlate || "N/A"}`,
        ],
    ...
    if (type === "os") {
      ...
      const splitDetails = doc.splitTextToSize(
        schedule.osDetails || "Nenhum detalhe informado.",
        180,
      );
      ...
      doc.text(
        schedule.osSignature || "___________________________________",
        14,
        finalY + 54,
      );
    } else {
      ...
      doc.line(14, finalY + 48, 100, finalY + 48);
      doc.text(
        tech?.name || "___________________________________",
        14,
        finalY + 54,
      );
    }
    ...
    doc.save(
      `${type === "os" ? "OS" : "Recibo"}_${schedule.vehiclePlate || "Servico"}.pdf`,
    );
  };
  ...
  const handleWorkflowAction = (
    ...
    switch (actionType) {
      ...
      case "confirm":
        ...
        if (!formData.date || !formData.time || !formData.technicianId) {
          addNotification(
            "error",
            "Dados Incompletos",
            "Defina Data, Hora e Técnico para confirmar.",
          );
          return;
        }
      ...
      case "onsite":
        const isPontoFixo =
          technicians.find((t) => t.id === schedule.technicianId)
            ?.serviceLocationType === "Ponto Fixo";
        newStatus = isPontoFixo ? "Cliente no local" : "Técnico no local";
        historyAction = isPontoFixo ? "Cliente no local" : "Técnico no local";
        historyDetails = isPontoFixo
          ? "Técnico informou chegada do cliente"
          : "Técnico informou chegada ao local";
        break;
      case "reschedule":
        ...
        historyDetails =
          "Solicitação retornada para pendente (Reagendamento necessário).";
        break;
      case "finish":
        if (!isFinishing) {
          setIsFinishing(true);
        ...
        if (!skipEquipment) {
          if (
            schedule.deviceType === "Tag" &&
            (!formData.installedTagImei ||
              formData.installedTagImei === "MANUAL")
          ) {
          ...
          if (
            schedule.deviceType === "Rastreador + Tag" &&
            (!formData.installedImei ||
              !formData.installedTagImei ||
              formData.installedTagImei === "MANUAL")
          ) {
            addNotification(
              "error",
              "Erro",
              "Informe o IMEI do Rastreador e o ID da Tag.",
            );
            return;
          }
        }
        ...
        historyDetails =
          "Equipamento informado, aguardando confirmação de vínculo.";
        break;
      case "cancel":
  ...
  const handleCopyClientMessage = () => {
    ...
    const dateDisplay = dateRaw
      ? getDisplayDate({ confirmedDate: dateRaw } as Schedule)
      : "A definir";
  ...
  const handleCopyTechnicianMessage = () => {
    ...
    const regionalName =
      companies.find((c) => c.id === formData.companyId)?.name || "N/A";
  ...
  const lastVerifier = [...schedule.history]
    .reverse()
    .find((h) => h.action === "Verificando" || h.action === "Assumiu");
  const isBeingVerifiedByMe =
    lastVerifier && lastVerifier.actionBy === currentUser?.name;
  const isBeingVerifiedByOther =
    lastVerifier && lastVerifier.actionBy !== currentUser?.name;
    ...
    return (
      <div className="fixed inset-0 z-[5000] flex items-center justify-center bg-black/60 backdrop-blur-sm p-2 sm:p-4">
        ...
        >
          ...
          <div className="w-full md:w-[60%] flex flex-col h-full bg-white dark:bg-zinc-900 text-zinc-900 dark:text-white relative border-r border-zinc-200 dark:border-zinc-800">
            <div className="p-6 border-b border-zinc-200 dark:border-zinc-800 flex justify-between items-center shrink-0 bg-white dark:bg-zinc-900 z-10">
              <h2 className="text-xl font-display font-black uppercase tracking-tight text-zinc-900 dark:text-white">
                Gerenciar Solicitação
              </h2>
              <button
                onClick={onClose}
                className="p-2 bg-zinc-100 dark:bg-zinc-800 hover:bg-zinc-200 dark:hover:bg-zinc-700 text-zinc-500 dark:text-zinc-400 hover:text-zinc-900 dark:hover:text-white rounded-xl transition-all"
              >
                <X size={20} />
              </button>
            </div>
            ...
            <div className="flex-1 overflow-y-auto custom-scrollbar flex flex-col relative">
              <div className="p-6 space-y-6">
                ...
                <div className="bg-zinc-50 dark:bg-zinc-950/50 rounded-3xl p-5 border border-zinc-200 dark:border-zinc-800">
                  <div className="flex justify-between items-start mb-4">
                    <div className="w-full pr-4">
                      <p className="text-[10px] font-black uppercase text-zinc-400 dark:text-zinc-500 tracking-widest mb-1">
                        Solicitante
                      </p>
                      <p className="text-sm font-bold text-zinc-900 dark:text-white uppercase mb-2">
                        {schedule.requesterName}
                      </p>

                      <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 md:gap-4 mt-4">
                        <div>
                          <label className="text-[9px] font-black uppercase text-zinc-400 tracking-wider mb-1 block">
                            Nome do Cliente
                          </label>
                          <input
                            type="text"
                            ...
                            className="w-full bg-white dark:bg-zinc-900 px-3 py-2 rounded-lg text-xs font-bold outline-none border border-zinc-200 dark:border-zinc-800 focus:border-primary-500 transition-colors"
                            placeholder="Nome"
                          />
                        </div>
                        <div>
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 md:gap-4 mb-2">
                    <div>
                      ...
                      <input
                        ...
                        className="w-full bg-white dark:bg-zinc-900 px-3 py-2 rounded-lg text-sm font-black uppercase outline-none border border-zinc-200 dark:border-zinc-800 focus:border-primary-500 transition-colors"
                      />
                    </div>
                    <div>
                      <label className="text-[9px] font-black uppercase text-zinc-400 tracking-wider mb-1 block">
                      ...
                      <input
                        ...
                        className="w-full bg-white dark:bg-zinc-900 px-3 py-2 rounded-lg text-xs font-bold uppercase outline-none border border-zinc-200 dark:border-zinc-800 focus:border-primary-500 transition-colors"
                      />
                    </div>
                    <div>
                      <label className="text-[9px] font-black uppercase text-zinc-400 tracking-wider mb-1 block">
                    ...
                    <div>
                      ...
                      <input
                        ...
                        onChange={(e) =>
                          setFormData({
                            ...formData,
                            vehicleColor: e.target.value.toUpperCase(),
                          })
                        }
                        className="w-full bg-white dark:bg-zinc-900 px-3 py-2 rounded-lg text-xs font-bold uppercase outline-none border border-zinc-200 dark:border-zinc-800 focus:border-primary-500 transition-colors"
                      />
                    ...
                    <div>
                      ...
                      <input
                        ...
                        onChange={(e) =>
                          setFormData({
                            ...formData,
                            vehicleStatus: e.target.value.toUpperCase(),
                          })
                        }
                        className="w-full bg-white dark:bg-zinc-900 px-3 py-2 rounded-lg text-xs font-bold uppercase outline-none border border-zinc-200 dark:border-zinc-800 focus:border-primary-500 transition-colors"
                      />
                    ...
                    <div>
                      ...
                      <input
                        ...
                        onChange={(e) =>
                          setFormData({
                            ...formData,
                            clientStatus: e.target.value.toUpperCase(),
                          })
                        }
                        className="w-full bg-white dark:bg-zinc-900 px-3 py-2 rounded-lg text-xs font-bold uppercase outline-none border border-zinc-200 dark:border-zinc-800 focus:border-primary-500 transition-colors"
                      />
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mt-4 bg-white dark:bg-zinc-900 p-3 rounded-xl border border-zinc-200 dark:border-zinc-800">
                    <div className="space-y-3">
                      <div>
                        <label className="text-[9px] font-black uppercase text-zinc-400 tracking-wider flex items-center gap-2 mb-1">
                        ...
                        <select
                          ...
                          className="w-full bg-zinc-100 dark:bg-zinc-950 px-2 py-1.5 rounded-lg text-xs font-bold outline-none border border-transparent focus:border-primary-500"
                        >
                      ...
                      <div>
                        <label className="text-[9px] font-black uppercase text-zinc-400 tracking-wider flex items-center gap-2 mb-1">
                          <TagIcon size={12} /> Tipo Equipamento
                        </label>
                        <select
                    ...
                    <div className="flex flex-col gap-3">
                      <Checkbox
                        ...
                        onChange={(checked) =>
                          setFormData({ ...formData, needsInspection: checked })
                        }
                      >
                      ...
                      <Checkbox
                        checked={formData.paymentOnSite}
                        onChange={(checked) =>
                          setFormData({ ...formData, paymentOnSite: checked })
                        }
                      >
                        <span className="text-[10px] font-bold text-zinc-500 uppercase">
                          Pagamento no Local
                    ...
                    <div className="mt-4 p-3 bg-emerald-50 dark:bg-emerald-900/10 rounded-xl border border-emerald-100 dark:border-emerald-900/20">
                      ...
                      <div className="grid grid-cols-2 gap-2">
                        {schedule.installedImei && (
                          <div>
                            <span className="text-[9px] font-black uppercase text-emerald-700 dark:text-emerald-500 tracking-wider mb-1 block">
                              Rastreador
                            </span>
                ...
                <div className="bg-amber-50 dark:bg-amber-900/10 p-4 rounded-3xl border border-amber-100 dark:border-amber-900/20">
                  <Checkbox
                    checked={formData.isRemoteLocation}
                    onChange={(checked) =>
                      setFormData({ ...formData, isRemoteLocation: checked })
                    }
                  >
                    <span className="text-[10px] font-black uppercase text-amber-700 dark:text-amber-400 tracking-widest flex items-center gap-2">
                      <Milestone size={14} /> Local Distante (Ativar Orçamento)
                    </span>
                  </Checkbox>
                  ...
                  {formData.isRemoteLocation && (
                    <div className="grid grid-cols-2 gap-4 mt-4 animate-in slide-in-from-top-2">
                      <div>
                        <label className="text-[9px] font-black uppercase text-amber-600 dark:text-amber-500 mb-1 block">
                          KM (Ida e Volta)
                        </label>
                        <div className="relative">
                          ...
                          <input
                            type="number"
                            value={formData.displacementKm}
                            onChange={(e) =>
                              setFormData({
                                ...formData,
                                displacementKm: parseFloat(e.target.value),
                              })
                            }
                            className="w-full pl-9 pr-3 py-2 bg-white dark:bg-zinc-950 border border-amber-200 dark:border-amber-900/50 rounded-xl text-xs font-bold outline-none focus:border-amber-500"
                            placeholder="0 KM"
                          />
                        </div>
                      </div>
                        ...
                        <div className="relative">
                          ...
                          <input
                            ...
                            onChange={(e) =>
                              setFormData({
                                ...formData,
                                displacementValue: parseFloat(e.target.value),
                              })
                            }
                            className="w-full pl-9 pr-3 py-2 bg-white dark:bg-zinc-950 border border-amber-200 dark:border-amber-900/50 rounded-xl text-xs font-bold outline-none focus:border-amber-500"
                            placeholder="R$ 0,00"
                ...
                <div className="space-y-4">
                  <div>
                    <label className="text-[9px] font-black uppercase text-zinc-400 dark:text-zinc-500 tracking-widest mb-1.5 block">
                      Atribuir Técnico
                    </label>
                    <div className="flex flex-col sm:flex-row gap-2">
                      <select
                        ...
                        className="flex-1 w-full sm:w-auto px-4 py-3 bg-zinc-100 dark:bg-zinc-950 border border-zinc-200 dark:border-zinc-800 rounded-xl font-bold text-sm text-zinc-900 dark:text-white outline-none focus:border-primary-500"
                      >
                      ...
                      <div className="flex gap-2 w-full sm:w-auto">
                          <button
                            ...
                            className="flex-1 sm:flex-none px-4 py-3 sm:py-0 bg-[#25D366] hover:bg-[#1fb550] text-white rounded-xl flex items-center justify-center shadow-lg transition-all active:scale-95"
                            title="Enviar para Técnico"
                          >
                            <Send size={18} />
                          </button>
                          <button
                            onClick={handleCopyTechnicianMessage}
                            className="flex-1 sm:flex-none px-4 py-3 sm:py-0 bg-zinc-100 dark:bg-zinc-800 hover:bg-zinc-200 dark:hover:bg-zinc-700 text-zinc-500 hover:text-zinc-900 dark:hover:text-white border border-zinc-200 dark:border-zinc-700 rounded-xl flex items-center justify-center transition-all active:scale-95"
                            title="Copiar Mensagem Técnico"
                          >
                            <Copy size={18} />
                          </button>
                          <button
                  ...
                  {formData.technicianId && (
                    <div className="bg-emerald-50 dark:bg-emerald-900/10 p-4 rounded-xl border border-emerald-100 dark:border-emerald-900/20">
                      <Checkbox
                        checked={formData.amountReceivedByTechnician > 0}
                        onChange={(checked) =>
                          setFormData({
                            ...formData,
                            amountReceivedByTechnician: checked
                              ? formData.amountReceivedByTechnician || 1
                              : 0,
                          })
                        }
                      >
                        <span className="text-[10px] font-black uppercase text-emerald-700 dark:text-emerald-400 tracking-widest flex items-center gap-2">
                          <Wallet size={14} /> Pagamento ao Técnico
                        </span>
                      </Checkbox>
                      ...
                      {formData.amountReceivedByTechnician > 0 && (
                        <div className="grid grid-cols-2 gap-4 mt-4 animate-in slide-in-from-top-2">
                          <div>
                            <label className="text-[9px] font-black uppercase text-emerald-600 dark:text-emerald-500 mb-1 block">
                              Valor Recebido (R$)
                            </label>
                            <div className="relative">
                              <DollarSign
                              ...
                              <input
                                type="number"
                                step="0.01"
                                value={formData.amountReceivedByTechnician}
                                onChange={(e) =>
                                  setFormData({
                                    ...formData,
                                    amountReceivedByTechnician: parseFloat(
                                      e.target.value,
                                    ),
                                  })
                                }
                                className="w-full pl-9 pr-3 py-2 bg-white dark:bg-zinc-950 border border-emerald-200 dark:border-emerald-900/50 rounded-xl text-xs font-bold outline-none focus:border-emerald-500"
                                placeholder="R$ 0,00"
                              />
                            </div>
                          </div>
                          <div>
                            ...
                            <input
                              type="date"
                              value={formData.technicianPaymentDate}
                              onChange={(e) =>
                                setFormData({
                                  ...formData,
                                  technicianPaymentDate: e.target.value,
                                })
                              }
                              className="w-full px-3 py-2 bg-white dark:bg-zinc-950 border border-emerald-200 dark:border-emerald-900/50 rounded-xl text-xs font-bold outline-none focus:border-emerald-500"
                            />
                          </div>
                        </div>
                      )}
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 md:gap-4">
                    <div>
                      <label className="text-[9px] font-black uppercase text-zinc-400 tracking-widest mb-1 block">
                        Data Confirmada
                      </label>
                      <input
                        type="date"
                        ...
                        className="w-full px-4 py-3 bg-zinc-100 dark:bg-zinc-950 border border-zinc-200 dark:border-zinc-800 rounded-xl font-bold text-sm text-zinc-900 dark:text-white outline-none focus:border-primary-500 transition-colors"
                      />
                    </div>
                    <div>
                      <label className="text-[9px] font-black uppercase text-zinc-400 tracking-widest mb-1 block">
                  ...
                  <div className="pt-6 border-t border-zinc-100 dark:border-zinc-800 space-y-3">
                    <p className="text-[9px] font-black uppercase text-zinc-400 tracking-widest mb-2">
                      Ações de Fluxo
                    </p>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                      ...
                      {schedule.status === "Solicitada" && (
                        <button
                          ...
                          className="py-4 bg-primary-500 text-black rounded-xl font-black uppercase text-xs tracking-widest hover:scale-[1.01] transition-all flex items-center justify-center gap-2 shadow-lg"
                        >
                      ...
                      {["Solicitada", "Em análise"].includes(
                        schedule.status,
                      ) && (
                        <>
                          <button
                            onClick={() => handleWorkflowAction("confirm")}
                            className="py-4 bg-emerald-500 text-white rounded-xl font-black uppercase text-xs tracking-widest hover:bg-emerald-600 transition-all flex items-center justify-center gap-2 shadow-lg"
                          >
                            <CheckCircle2 size={16} /> Confirmar Agendamento
                          </button>

                          {schedule.status === "Em análise" &&
                            isBeingVerifiedByOther && (
                              <button
                                onClick={() => handleWorkflowAction("verify")}
                                className="py-4 bg-amber-500 text-white rounded-xl font-black uppercase text-xs tracking-widest hover:bg-amber-600 transition-all flex items-center justify-center gap-2 shadow-sm"
                              >
                                <Activity size={16} /> Assumir (De:{" "}
                                {lastVerifier?.actionBy})
                              </button>
                            )}

                          <button
                            onClick={() => handleWorkflowAction("budget")}
                            className="py-4 bg-zinc-100 dark:bg-zinc-800 text-zinc-600 dark:text-zinc-300 rounded-xl font-black uppercase text-xs tracking-widest hover:bg-zinc-200 dark:hover:bg-zinc-700 transition-all flex items-center justify-center gap-2 border border-zinc-200 dark:border-zinc-700"
                          >
                      ...
                      {schedule.status === "Em orçamento" && (
                        <button
                          onClick={() => handleWorkflowAction("confirm")}
                          className="py-4 bg-emerald-500 text-white rounded-xl font-black uppercase text-xs tracking-widest hover:bg-emerald-600 transition-all flex items-center justify-center gap-2 shadow-lg col-span-2"
                        >
                        ...
                        !isFinishing && (
                          <>
                            ...
                            {schedule.status !== "Técnico no local" &&
                              schedule.status !== "Cliente no local" && (
                                <button
                                  onClick={() => handleWorkflowAction("onsite")}
                                  className="py-4 bg-blue-500 text-white rounded-xl font-black uppercase text-xs tracking-widest hover:bg-blue-600 transition-all flex items-center justify-center gap-2 shadow-lg"
                                >
                                  <MapPin size={16} />{" "}
                                  {technicians.find(
                            ...
                            <button
                              ...
                              className="py-4 bg-emerald-600 text-white rounded-xl font-black uppercase text-xs tracking-widest hover:bg-emerald-700 transition-all flex items-center justify-center gap-2 shadow-lg"
                            >
                            ...
                            <button
                              onClick={() => handleWorkflowAction("reschedule")}
                              className="py-4 bg-amber-100 dark:bg-amber-900/30 text-amber-700 dark:text-amber-400 rounded-xl font-black uppercase text-xs tracking-widest hover:bg-amber-200 dark:hover:bg-amber-900/50 transition-all flex items-center justify-center gap-2 border border-amber-200 dark:border-amber-800"
                            >
                              <RotateCcw size={16} /> Reagendar
                            </button>
                            <button
                              onClick={() => handleWorkflowAction("frustrate")}
                              className="py-4 bg-zinc-100 dark:bg-zinc-800 text-zinc-600 dark:text-zinc-300 rounded-xl font-black uppercase text-xs tracking-widest hover:bg-zinc-200 dark:hover:bg-zinc-700 transition-all flex items-center justify-center gap-2 border border-zinc-200 dark:border-zinc-700"
                            >
                      ...
                      {isFinishing && (
                        <div className="col-span-1 md:col-span-2 space-y-4 bg-emerald-50 dark:bg-emerald-900/10 p-4 rounded-xl border border-emerald-200 dark:border-emerald-900/30">
                          <h4 className="text-[10px] font-black uppercase text-emerald-600 dark:text-emerald-400 tracking-widest">
                          ...
                          <div className="flex items-center gap-2 mb-2">
                            ...
                            >
                              <span className="text-[10px] font-bold text-emerald-900 dark:text-emerald-500 uppercase tracking-wider">
                                Finalizar sem vincular equipamento
                              </span>
                            </Checkbox>
                          </div>
                          {!skipEquipment &&
                            (formData.deviceType === "Rastreador" ||
                              formData.deviceType === "Rastreador + Tag") && (
                              <input
                                type="text"
                                value={formData.installedImei}
                                onChange={(e) =>
                                  setFormData({
                                    ...formData,
                                    installedImei: e.target.value,
                                  })
                                }
                                className="w-full px-4 py-3 bg-white dark:bg-zinc-900 border border-emerald-200 dark:border-emerald-800 rounded-xl font-bold text-sm outline-none"
                                    ...
                                    <div className="flex-1 w-full relative">
                                      <SearchableSelect
                                        ...
                                        onChange={(val) =>
                                          setFormData({
                                            ...formData,
                                            installedTagImei: val,
                                          })
                                        }
                                        placeholder="Selecione uma Tag no estoque..."
                                    ...
                                    <div className="flex w-full gap-2 relative">
                                      <input
                                        ...
                                        onChange={(e) =>
                                          setFormData({
                                            ...formData,
                                            installedTagImei: e.target.value,
                                          })
                                        }
                                        className="w-full px-4 py-3 bg-white dark:bg-zinc-900 border border-emerald-200 dark:border-emerald-800 rounded-xl font-bold text-sm outline-none shadow-sm"
                                      ...
                                      <button
                                        ...
                                        className="absolute right-3 top-1/2 -translate-y-1/2 p-2 hover:bg-emerald-50 dark:hover:bg-emerald-900/30 text-emerald-600 rounded-lg"
                                      >
                                        <X size={16} />
                                      </button>
                                    </div>
                          ...
                          <div className="flex gap-2 pt-2">
                            <button
                              ...
                              className="flex-1 py-3 bg-zinc-200 dark:bg-zinc-800 text-zinc-600 dark:text-zinc-300 rounded-xl font-bold text-xs uppercase"
                            >
                              Cancelar
                            </button>
                            <button
                              onClick={() => handleWorkflowAction("finish")}
                              className="flex-1 py-3 bg-emerald-600 text-white rounded-xl font-bold text-xs uppercase shadow-lg flex items-center justify-center gap-2"
                            >
                    ...
                    {schedule.status === "Aguardando Vínculo" && (
                      <div className="mt-4 p-4 border border-violet-200 dark:border-violet-900/30 bg-violet-50 dark:bg-violet-900/10 rounded-2xl flex flex-col gap-3">
                        <div className="flex items-start gap-3">
                          ...
                          />
                          <div>
                            <h4 className="text-[10px] font-black uppercase text-violet-700 dark:text-violet-400 tracking-widest">
                              Aguardando Vínculo
                            </h4>
                            <p className="text-xs font-medium text-violet-600 dark:text-violet-300 mt-1">
                              O técnico finalizou o serviço. Confirme o vínculo
                              do equipamento ao veículo.
                            </p>
                          </div>
                        </div>
                        <button
                          onClick={handleConfirmarEquipamentoClick}
                        ...
                        >
                          <CheckCircle2 size={16} /> Confirmar Equipamento
                          Vinculado
                        </button>
                      </div>
                    )}
                      ...
                      <button
                        onClick={() =>
                          printOsDoc(
                            schedule,
                            technicians.find(
                              (t: any) => t.id === schedule.technicianId,
                            ),
                            companies.find(
                              (c: any) => c.id === schedule.companyId,
                            ),
                          )
                        }
                        className="w-full py-3 bg-zinc-800 text-white dark:bg-white dark:text-zinc-900 rounded-xl font-black uppercase text-[10px] tracking-widest hover:scale-[1.01] transition-all flex items-center justify-center gap-2"
                      >
                        <FileText size={14} /> Imprimir OS (PDF)
                      </button>
                      <div className="flex gap-2">
                        <button
                          onClick={handleAdminDataSave}
                          className="flex-1 py-3 bg-zinc-100 dark:bg-zinc-800 text-zinc-600 dark:text-zinc-300 rounded-xl font-black uppercase text-[10px] tracking-widest hover:bg-zinc-200 dark:hover:bg-zinc-700 transition-all flex items-center justify-center gap-2"
                        >
                          <Save size={14} /> Salvar
                        </button>
                        ...
                        {schedule.status !== "Cancelada" &&
                          ...
                          schedule.status !== "Aguardando Vínculo" && (
                            <button
                              ...
                              className="flex-1 py-3 bg-red-100 dark:bg-red-900/20 text-red-600 dark:text-red-400 rounded-xl font-black uppercase text-[10px] tracking-widest hover:bg-red-200 dark:hover:bg-red-900/40 transition-all flex items-center justify-center gap-2"
                            >
                        ...
                        {onDelete && (
                          <button
                            onClick={() => handleWorkflowAction("delete")}
            ...
            <div className="flex-1 overflow-y-auto p-6 space-y-6">
              {schedule.history
                ...
                .map((h, i) => (
                  <div
                    key={i}
                    className="relative pl-6 border-l border-zinc-200 dark:border-zinc-800 last:border-0 pb-2"
                  >
                    ...
                    <div className="flex flex-col gap-1">
                      <div className="flex justify-between items-center">
                        <span className="text-[10px] font-black uppercase text-zinc-900 dark:text-white">
                          {h.action}
                        </span>
        ...
        <ConfirmModal
          ...
          onConfirm={() => completeService(true)}
          onCancel={() => completeService(false)}
          title="Novo Cadastro"
          message="O cliente ou veículo ainda não estão cadastrados na base. Deseja cadastrá-los automaticamente agora ou deseja apenas finalizar sem cadastrar?"
          type="success"
          confirmText="Sim, cadastrar"
          cancelText="Apenas concluir sem cadastro"
        />
      </div>
    );
  }

  if (currentUser?.role === "technician" || currentUser?.role === "admin_tecnico") {
    return (
      <TechnicianTaskModal 
        schedule={schedule}
        ...
        <div className="p-6 border-b border-zinc-100 dark:border-zinc-800 flex justify-between items-center bg-zinc-50/50 dark:bg-zinc-950/50">
          <div>
            <h2 className="text-lg font-display font-black uppercase tracking-tight text-zinc-900 dark:text-white">
              {isUserEditing ? "Editar Solicitação" : "Detalhes do Serviço"}
            </h2>
          ...
          {isUserEditing ? (
            <div className="space-y-4 bg-zinc-50 dark:bg-zinc-950/50 p-4 rounded-2xl border border-zinc-100 dark:border-zinc-800">
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="text-[10px] font-black uppercase text-zinc-400 tracking-wider mb-1 block">
                    Nome do Cliente
                  </label>
                  <input
                    type="text"
                    ...
                    className="w-full px-3 py-2 bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-700 rounded-xl text-xs font-bold outline-none"
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <input
                    ...
                    className="w-full px-3 py-2 bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-700 rounded-xl text-xs font-black uppercase outline-none"
                  />
                </div>
                <div>
                  <label className="text-[10px] font-black uppercase text-zinc-400 tracking-wider mb-1 block">
                  ...
                  <input
                    ...
                    className="w-full px-3 py-2 bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-700 rounded-xl text-xs font-bold uppercase outline-none"
                  />
                </div>
              </div>
              <div>
              ...
              <div>
                ...
                <textarea
                  ...
                  className="w-full px-3 py-2 bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-700 rounded-xl text-xs font-bold outline-none min-h-[80px]"
              ...
              <div className="flex gap-2 pt-2">
                ...
                <button
                  ...
                  className="flex-1 py-3 bg-primary-500 text-black rounded-xl font-bold text-xs uppercase shadow-lg"
                >
                  Salvar
                </button>
              </div>
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div className="p-4 bg-zinc-50 dark:bg-zinc-950/50 rounded-2xl border border-zinc-100 dark:border-zinc-800 col-span-2 sm:col-span-1">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <p
                      className={`text-sm font-bold uppercase ${assignedTech ? "text-zinc-900 dark:text-white" : "text-zinc-400 italic"}`}
                    >
                    ...
                    <div className="space-y-3">
                        ...
                        <div className="grid grid-cols-1 gap-2">
                          <div>
                            ...
                            <input
                              ...
                              className="w-full px-2 py-1.5 bg-white dark:bg-zinc-950 border border-emerald-200 dark:border-emerald-900/50 rounded-lg text-xs font-bold outline-none focus:border-emerald-500"
                              placeholder="0,00"
                            />
                          </div>
                          <div>
                      ...
                      <div className="flex gap-2 pt-2">
                        <button
                          ...
                          className="flex-1 py-2 bg-zinc-200 dark:bg-zinc-800 text-zinc-600 dark:text-zinc-300 rounded-lg font-bold text-[10px] uppercase"
                        >
                          Cancelar
                        </button>
                        <button
                          onClick={handleUserPaymentSave}
                          className="flex-1 py-2 bg-emerald-500 text-white rounded-lg font-bold text-[10px] uppercase shadow-md"
                        >
                          Salvar
                        </button>
                      </div>
                ...
                ) : schedule.amountReceivedByTechnician &&
                  schedule.amountReceivedByTechnician > 0 ? (
                ...
                <div className="p-4 bg-zinc-50 dark:bg-zinc-950/50 rounded-2xl border border-zinc-100 dark:border-zinc-800 col-span-2">
                  ...
                  <p
                    className="text-xs font-bold text-zinc-900 dark:text-white line-clamp-2"
                    title={schedule.locationAddress}
                  >
                  ...
                  <a
                    ...
                    target="_blank"
                    rel="noopener noreferrer"
                    className="text-[9px] font-black text-[#f59e0b] uppercase tracking-widest inline-flex items-center gap-1 hover:underline mt-2"
                  >
                    Ver no Mapa <ExternalLink size={10} />
                  </a>
                ...
                {(schedule.installedImei || schedule.installedTagImei) && (
                  <div className="p-4 bg-emerald-50 dark:bg-emerald-900/10 rounded-2xl border border-emerald-100 dark:border-emerald-900/20 col-span-2">
                    ...
                    <div className="flex flex-col gap-1">
                      ...
                      {schedule.installedTagImei && (
                        <p className="text-xs font-bold text-emerald-900 dark:text-emerald-300">
                          <span className="opacity-70 font-medium">Tag:</span>{" "}
                          {schedule.installedTagImei}
                        </p>
                      )}
                    </div>
                  ...
                  <div className="mb-4 p-4 border border-violet-200 dark:border-violet-900/30 bg-violet-50 dark:bg-violet-900/10 rounded-2xl flex flex-col gap-3">
                    <div className="flex items-start gap-3">
                      ...
                      <div>
                        ...
                        <p className="text-xs font-medium text-violet-600 dark:text-violet-300 mt-1">
                          O técnico finalizou o serviço. Confirme o vínculo do
                      ...
                      >
                        ...
                        <div className="flex flex-col gap-0.5">
                          <div className="flex justify-between items-center">
                            ...
                            <span className="text-[9px] font-mono text-zinc-400">
                              {new Date(h.timestamp).toLocaleDateString()} às{" "}
                ...
                <div className="pt-4 border-t border-zinc-100 dark:border-zinc-800 space-y-3">
                  <button
                    onClick={() =>
                      printOsDoc(
                        ...
                        companies.find((c: any) => c.id === schedule.companyId),
                      )
                    }
                    className="w-full py-4 bg-zinc-800 text-white dark:bg-white dark:text-zinc-900 rounded-2xl text-xs font-black uppercase tracking-widest transition-all flex items-center justify-center gap-2 shadow-lg hover:scale-[1.02]"
                  >
                    <FileText size={16} /> Imprimir OS (PDF)
                  </button>
                  {["Confirmada", "Reagendada", "Autorizada"].includes(
                    schedule.status,
                  ) && (
                    <button
                      onClick={() => {
                        ...
                        const newStatus = isPontoFixo
                          ? "Cliente no local"
                          : "Técnico no local";
                        const actionText = isPontoFixo
                          ? "Cliente no local"
                          : "Chegou no local";
                        const detailsText = isPontoFixo
                          ? "Técnico informou chegada do cliente"
                          : "Técnico informou chegada ao local";
                  ...
                  {(schedule.status === "Técnico no local" ||
                    ...
                    <div className="space-y-3 p-4 bg-emerald-50 dark:bg-emerald-900/10 border border-emerald-100 dark:border-emerald-900/20 rounded-2xl">
                      <h4 className="text-[10px] font-black uppercase text-emerald-600 dark:text-emerald-500 tracking-widest mb-2">
                        Finalizar Serviço
                      </h4>
                      ...
                      {isFinishing ? (
                        <div className="space-y-3">
                          ...
                          {!skipEquipment &&
                            (schedule.deviceType === "Rastreador" ||
                              schedule.deviceType === "Rastreador + Tag") && (
                              <div className="flex gap-2">
                                <input
                                  type="text"
                                  ...
                                  className="flex-1 px-4 py-3 bg-white dark:bg-zinc-900 border border-emerald-200 dark:border-emerald-800 rounded-xl font-bold text-sm outline-none"
                                  placeholder="IMEI do Rastreador"
                                />
                                <button
                                  onClick={() => setScannerActive("imei")}
                                  className="p-3 bg-emerald-100 dark:bg-emerald-900/30 text-emerald-600 dark:text-emerald-400 rounded-xl"
                                >
                                  <Camera size={20} />
                                </button>
                              </div>
                                ...
                                <div className="flex gap-2">
                                  ...
                                  <button
                                    onClick={() => {
                                      setFormData({
                                        ...formData,
                                        installedTagImei: "MANUAL",
                                      });
                                      setScannerActive("tag");
                                    }}
                                    className="p-3 bg-emerald-100 dark:bg-emerald-900/30 text-emerald-600 dark:text-emerald-400 rounded-xl"
                                  >
                                    <Camera size={20} />
                          ...
                          {scannerActive && (
                            <div className="fixed inset-0 z-[6000] bg-black flex flex-col items-center justify-center p-4">
                              <div
                                ...
                                className="w-full max-w-md aspect-square bg-black"
                              ></div>
                              <button
                                onClick={() => {
                                  if (
                                ...
                                className="mt-4 px-6 py-3 bg-zinc-700 text-white rounded-xl font-black uppercase"
                              >
                                Cancelar
                              </button>
                            </div>
                          ...
                          <div className="space-y-2 pt-2 border-t border-emerald-200 dark:border-emerald-800">
                            <h5 className="text-[10px] font-black uppercase text-emerald-700 dark:text-emerald-400">
                              Ordem de Serviço
                            </h5>
                            <input
                              type="text"
                            ...
                            />
                            <textarea
                              value={formData.osDetails}
                              onChange={(e) =>
                                setFormData({
                                  ...formData,
                                  osDetails: e.target.value,
                                })
                              }
                              className="w-full px-4 py-3 bg-white dark:bg-zinc-900 border border-emerald-200 dark:border-emerald-800 rounded-xl font-bold text-sm outline-none resize-none h-24"
                              placeholder="Detalhes do serviço executado..."
                            />

                            <div className="mt-4">
                              <h5 className="text-[10px] font-black uppercase text-emerald-700 dark:text-emerald-400 mb-2">
                              ...
                              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                {formData.checklist.map((item, index) => (
                                  <div
                                    key={index}
                                    className="bg-white dark:bg-zinc-900 border border-emerald-100 dark:border-emerald-800/50 rounded-xl p-3 flex flex-col gap-2"
                                  >
                                    <span className="text-xs font-bold text-zinc-700 dark:text-zinc-300">
                                      {item.name}
                                    </span>
                            ...
                            <input
                              type="text"
                              value={formData.osSignature}
                              onChange={(e) =>
                                setFormData({
                                  ...formData,
                                  osSignature: e.target.value,
                                })
                              }
                              className="w-full px-4 py-3 bg-white dark:bg-zinc-900 border border-emerald-200 dark:border-emerald-800 rounded-xl font-bold text-sm outline-none mt-4"
                              placeholder="Nome do Responsável (Assinatura)"
                            />
                          </div>
                                ...
                                const updated = {
                                  ...
                                  history: [
                                    ...schedule.history,
                                    {
                                      ...
                                      timestamp: Date.now(),
                                      details:
                                        "Técnico finalizou e concluiu o serviço",
                                    },
                                  ],
                                };
                                onUpdate(updated);
                  ...
                  {schedule.status === "Concluída" && (
                    <div className="space-y-3 p-4 bg-blue-50 dark:bg-blue-900/10 border border-blue-100 dark:border-blue-900/20 rounded-2xl">
                      <h4 className="text-[10px] font-black uppercase text-blue-600 dark:text-blue-500 tracking-widest mb-2">
                        Documentos
                      </h4>
                      <div className="flex gap-2">
                        <button
